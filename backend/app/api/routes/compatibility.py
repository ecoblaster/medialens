...
def _profile_or_404(device_id: str):
...
def devices() -> list[DeviceProfileRead]:
    return [profile.read_model() for profile in list_device_profiles()]
...
def compatibility_summary(
    ...
    evaluations = [evaluate_file(media_file, profile) for media_file in db.scalars(query).unique().all()]
...
def compatibility_files(
    ...
    evaluations = [evaluate_file(media_file, profile) for media_file in db.scalars(query).unique().all()]
    if outcome:
        evaluations = [evaluation for evaluation in evaluations if evaluation.outcome == outcome]
